-- Fixed Supabase Database Setup for EcoLink
-- This script aligns with the existing database schema
-- Run this in your Supabase SQL Editor to fix the database

-- 1. Enable necessary extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 2. Update profiles table to match expected schema
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'general_user' CHECK (role IN ('admin', 'verifier', 'project_developer', 'buyer_investor', 'general_user'));
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS kyc_level INTEGER DEFAULT 0 CHECK (kyc_level >= 0 AND kyc_level <= 3);

-- 3. Update projects table to match expected schema
...
ALTER TABLE projects ADD CONSTRAINT IF NOT EXISTS projects_status_check CHECK (status IN ('pending', 'under_review', 'approved', 'rejected', 'active'));

-- 4. Update project_credits table to match expected schema
...
ALTER TABLE project_credits ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'active' CHECK (status IN ('active', 'sold_out', 'retired'));

-- 5. Update credit_listings table to match expected schema
-- Note: The existing table uses project_credits_id, not project_credit_id
ALTER TABLE credit_listings ADD COLUMN IF NOT EXISTS listed_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
ALTER TABLE credit_listings ADD CONSTRAINT IF NOT EXISTS credit_listings_status_check CHECK (status IN ('active', 'sold_out', 'cancelled'));

-- 6. Create credit_transactions table if it doesn't exist
...
-- 7. Create credit_ownership table if it doesn't exist
...
-- 9. Create wallets table (using wallet_accounts as base)
...
-- 10. Create wallet_transactions table (using existing wallet_transactions structure)
-- Note: The existing table uses account_id, not wallet_id
ALTER TABLE wallet_transactions ADD COLUMN IF NOT EXISTS type TEXT CHECK (type IN ('deposit', 'withdrawal', 'purchase', 'sale', 'refund'));
...
ALTER TABLE wallet_transactions ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'pending' CHECK (status IN ('pending', 'completed', 'failed', 'cancelled'));

-- 11. Create certificates table if it doesn't exist
...
-- 12. Create receipts table if it doesn't exist
...
-- 14. Create RLS Policies (using correct column names)
...
-- 15. Create functions (using correct column names)
...
-- 18. Success message
SELECT 'Fixed Supabase setup completed! Database schema is now aligned with existing structure.' as status;
