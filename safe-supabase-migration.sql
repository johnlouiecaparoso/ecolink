-- Safe Supabase Database Migration for EcoLink
-- This script safely migrates existing data and adds constraints step by step
-- Run this in your Supabase SQL Editor

-- 1. Enable necessary extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 2. DIAGNOSTIC: Check current role values (for reference)
-- Uncomment these lines to see what roles exist before migration:
...
-- 8. Create credit_transactions table if it doesn't exist
...
-- 9. Create credit_ownership table if it doesn't exist
...
-- 10. Create audit_logs table if it doesn't exist
...
-- 11. Create wallets table (using wallet_accounts as base)
...
-- 12. Update wallet_transactions table (using existing wallet_transactions structure)
...
-- SAFE MIGRATION for wallet_transactions type constraint
-- Step A: Temporarily allow both old and new type values
...
-- Step C: Now safely enforce only new type values
ALTER TABLE wallet_transactions DROP CONSTRAINT IF EXISTS wallet_transactions_type_check;
ALTER TABLE wallet_transactions ADD CONSTRAINT wallet_transactions_type_check CHECK (type IN (
...
-- 13. Create certificates table if it doesn't exist
...
-- 14. Create receipts table if it doesn't exist
...
-- 16. Create RLS Policies (using correct column names)
...
-- 17. Create functions (using correct column names and proper PL/pgSQL structure)
...
-- 20. VERIFICATION: Check that migration was successful
-- Uncomment these lines to verify the migration worked:
-- SELECT role, COUNT(*) FROM profiles GROUP BY role ORDER BY COUNT(*) DESC;
-- SELECT 'Migration completed successfully!' as status;

-- 21. Success message
SELECT 'Safe Supabase migration completed! All constraint violations resolved and database schema aligned.' as status;
